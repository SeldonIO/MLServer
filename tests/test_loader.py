...
def test_list(
    sum_model_settings: ModelSettings, model_settings_loader: ModelSettingsLoader
):
    settings_list = model_settings_loader.list()
    ...
    assert settings_list[0].parameters.uri == str(model_settings_loader._root)


def test_list_multi_model(
    multi_model_folder: str,
):
    multi_model_loader = ModelSettingsLoader(multi_model_folder)
...
def test_list_fallback(
    ...
    all_settings = model_settings_loader.list()
