...
# Configurable values
BASE_PACKAGE = "mlserver"
PYTHON_SRC = Path("python")
DOCS_DIR = Path("docs-gb/api")
SUMMARY_FILE = Path("docs-gb/api/SUMMARY.md")
...
def generate_all_docs():
    ...
    for modname, module in walk_package(BASE_PACKAGE):
        ...
        print(f"ðŸ“„ Generating: {rel_path}")
        out_path.write_text(document_module(module, modname))
        summary_lines.append(f"  * [{modname}](api/{rel_path})\n")

    SUMMARY_FILE.write_text("".join(summary_lines))
    print(f"\nâœ… Docs written to: {DOCS_DIR}/")
    print(f"ðŸ“š Sidebar: {SUMMARY_FILE}")
