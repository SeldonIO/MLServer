...
def case_tree_shap(sk_income_model_uri, income_data, tmp_path) \
    ...
    return build_test_case('tree_shap', init_kwargs, explain_kwargs, fit='no-data',  # fit w/o data i.e. path-dep. algo
                           save_dir=tmp_path, model_uri=sk_income_model_uri, payload=income_data['X'][:1])


@pytest.mark.parametrize('save', [True, False])
def case_tree_partial_dependence(sk_income_model_uri, income_data, tmp_path, save) \
        -> Tuple[ModelSettings, Explainer, InferenceRequest, dict]:
    """
    ...
    return build_test_case('tree_partial_dependence', init_kwargs, explain_kwargs, fit=False,
                           save_dir=save_dir, model_uri=sk_income_model_uri, payload=income_data['X'][:1])


def case_tree_partial_dependence_variance(sk_income_model_uri, income_data) \
        -> Tuple[ModelSettings, Explainer, InferenceRequest, dict]:
    """
    ...
    return build_test_case('tree_partial_dependence', init_kwargs, explain_kwargs, fit=False,
                           save_dir=None, model_uri=sk_income_model_uri, payload=income_data['X'][:1])
