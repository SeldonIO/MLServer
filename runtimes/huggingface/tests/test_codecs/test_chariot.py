...
@pytest.mark.parametrize(
    "task_type,class_int_to_str,predict_proba, hf_prediction,expected_chariot_output",
    [
        (
        ...
        (
            "image-classification",
            {0: "Egyptian cat", 1: "tabby, tabby cat", 2: "tiger cat"},
            True,
            [
                [
                    {"label": "tabby, tabby cat", "score": 0.94},
            ...
            [
                [
                    ...
                    },
                    {
                        ...
                        "label": "class_1",
                        "mask": Image.fromarray(
                            np.array([[0, 0, 0], [0, 255, 255], [0, 0, 0]]).astype(
                                "uint8"
                            ),
                            mode="L",
                        ),
                    },
                    {
                        ...
                        "label": "class_2",
                        "mask": Image.fromarray(
                            np.array([[0, 0, 0], [0, 0, 0], [0, 255, 0]]).astype(
                                "uint8"
                            ),
                            mode="L",
                        ),
...
def test_encode_input(
    ...
    chariot_output = ChariotImgModelOutputCodec.encode_output(
        hf_prediction, task_type, class_int_to_str, predict_proba
    )
