...
images, etc.
Unfortunately, the definition of the [V2 Inference Protocol](https://docs.seldon.io/projects/seldon-core/en/latest/reference/apis/v2-protocol.html) 
doesn't cover any of the specific use cases.
...
![Content Types](../images/content-type.svg)
...
specify it through the `content_type` field of the `parameters` section of
your request.
...
`InferenceRequest <mlserver.types.InferenceRequest>` or
`InferenceResponse <mlserver.types.InferenceResponse>`).
...
kwargs to the class' constructor (or use any of the [other methods](https://docs.pydantic.dev/latest/usage/models/#model-properties)
...
- The input / output entry containing NaN values uses either the `FP16`, `FP32`
  or `FP64` datatypes.
- You are either using the [Pandas codec](#pandas-dataframe) or the [Numpy codec](#numpy-array).
