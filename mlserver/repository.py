...
class SingleModelRepository:
    """
    Repository for a single model with multiple versions.
    """
...
class MultiModelRepository:
    """
    Multiple model repository, where each model can have multiple versions.
    """
    ...
    async def load(self, model: MLModel):
        if model.name not in self._models:
            self._models[model.name] = SingleModelRepository(model)
